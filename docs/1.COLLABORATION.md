...
- `curl`, `jq`, and `netstat` utilities for debugging and testing
...
- ERROR: "Missing dependencies: curl jq docker" | Install required dependencies: `brew install curl jq` on macOS or `apt-get install curl jq` on Ubuntu
